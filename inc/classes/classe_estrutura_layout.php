...
#param_url
	...
	#se a subsecao tem vinculo direto com uma seção
	if(!empty($dados['sec_pai'])){
		$sec_pai = consultaDadosTabela('sec','sec',array('sec_id'=>$dados['sec_pai'],''),'');
		$this->dados['url']['href'] = $this->dados['url']['inicio'].'/'.$sec_pai[0]['sec'];
	}
	#alvo				
	if(!empty($dados['alvo'])){								
		...
		$this->dados['url']['alvo']['param_url']=explode(',',$param_url_alvo[0]['param_url']);		
		 ...
		 $this->dados['url']['href'] = $this->dados['url']['href']."/".$dados['sub'];	
...
public function setDado(){
	...
	}elseif(isset($dadosParam[$param])){	
        $this->dado = $dadosParam[$param];
	}elseif($dadosMetodo = $this->getDadosMetodo()){		
		if(isset($dadosMetodo[$param])AND $param != "src"){
			$this->dado = mascaraDados($dadosParam['mascara'],$dadosMetodo[$param]);						
		}	
	}else{
		$dado = $this->getDados();
		if(isset($dado['sec'][$param])){
			$this->dado = $dado['sec'][$param];
		}
	}			
}
